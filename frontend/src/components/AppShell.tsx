...
export async function AppShell({ app, logo, alt, children, groups }: AppShellProps) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="side-brand"><Image src={logo} alt={alt} width={36} height={36} /><div><strong>{app}</strong><small>Business OS</small></div></div><Link href="/apps" className="back-link">Back to Business OS</Link><AppNavigation groups={navigationGroups} /></aside><section className="workspace"><div className="global-notification-bar"><NotificationBell unreadCount={unreadCount ?? 0} recent={recent ?? []}/><details className="account-menu"><summary><span className="account-avatar">{initials}</span><span className="account-name">{displayName}</span></summary><div className="account-menu-panel"><strong>{displayName}</strong><span>{user.user?.email}</span><Link href="/settings/profile">Edit profile</Link><Link href="/settings/notifications">Notification preferences</Link></div></details></div>{children}</section></div>;
}
