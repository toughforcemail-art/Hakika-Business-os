...
export function RealEstateShell({ children }: { children: React.ReactNode }) {
  return <div className="re-layout"><aside className="re-shell-sidebar"><div className="re-brand"><Image src="/brands/real-estate/logo.jpg" alt="Hakika Real Estate" width={40} height={40}/><div><strong>Hakika Real Estate</strong><small>Business OS application</small></div></div><Link href="/apps" className="re-shell-back">← Back to Business OS</Link><nav className="re-shell-nav">{realEstateNavigationGroups.map((group, index) => <details className="re-nav-group" key={group.label} open={index === 0}><summary>{group.label}</summary>{group.links.map((link) => <Link href={link.href} key={link.href}>{link.label}</Link>)}</details>)}</nav></aside><section className="re-layout-workspace"><div className="re-mobile-topbar"><MobileNav/><span>Hakika Real Estate</span></div>{children}</section></div>;
}
