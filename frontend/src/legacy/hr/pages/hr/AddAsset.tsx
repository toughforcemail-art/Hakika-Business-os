...
const AddAsset: React.FC = () => {
  ...
  const fetchCatalog = async () => {
    try {
      const { data: cData } = await supabase.from('hr_asset_catalog').select('*').order('model_name');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        setTimeout(() => navigate('/app/hr/assets'), 1200);
      } else {
        ...
        const { error } = await supabase.from('hr_assets').insert(assetsToInsert);
        if (error) throw error;
        ...
        setTimeout(() => navigate('/app/hr/assets'), 1500);
      }
    } catch (error) {
    ...
    <div className="min-h-full w-full p-6 lg:p-10 bg-white dark:bg-dark-bg text-gray-900 dark:text-white">
      ...
      <div className="max-w-xl mx-auto space-y-8">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/app/hr/assets')} className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-full transition-colors" title="Go back to Assets list">
            <ArrowLeft size={24} />
          </button>
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-dark-text">
              {isEditing ? 'Update the asset details or remove the record from inventory.' : 'Add equipment or gear to HR inventory'}
            </p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border rounded-3xl p-8 shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-1">
              ...
              <input required list="catalog-suggestions" placeholder="e.g. Laptop, Desk, Monitor..." className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl p-3 text-sm focus:ring-2 focus:ring-brand-purple outline-none" value={formData.name} onChange={(e) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <AddableSelect label="Equipment Type" tableName="hr_asset_types" value={formData.type} onChange={(val) => setFormData({...formData, type: val})} required allowCustomOption={true} />
               <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-400 uppercase">Color / Finish</label>
                  <input className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl p-3 text-sm focus:ring-2 focus:ring-brand-purple outline-none" value={formData.color} onChange={(e) => setFormData({...formData, color: e.target.value})} placeholder="e.g. Black, Silver..." />
               </div>
               <AddableSelect label="Condition" tableName="hr_asset_conditions" value={formData.condition} onChange={(val) => setFormData({...formData, condition: val})} required allowCustomOption={true} />
            </div>
            ...
            <div className="flex justify-end gap-3 pt-6">
              ...
              <button type="button" onClick={() => navigate('/app/hr/assets')} className="px-8 py-3 text-sm font-bold text-gray-500 hover:bg-gray-100 rounded-xl transition-all">Cancel</button>
